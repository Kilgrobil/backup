...
Jusqu'à présent, les catégories ont l'air d'être assez disjointes les unes des autres. On peut en fait les munir de transformations du type $\gr{C} \to \gr{D}$, nommées \il{foncteurs}.
\begin{defi}[Foncteur]
...
Soit $F,G : \gr{C} \to \gr{D}$ deux foncteurs. Une transformation naturelle $\lambda$ consiste en la donnée, pour tout $X \in \gr{C}$, d'un foncteur $\lambda_X : F(X) \to G(X)$. Ces foncteurs sont tels que pour tout $f : X \to Y$, le diagramme suivant commute :
\begin{center}
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
  {
...
\[ M' \models_{\Sigma'} Sen(\sigma)(\ph) \iff Mod(\sigma)(M') \models_{\Sigma} \ph \]
\end{itemize}
\end{defi}
Une institution est la formalisation de ce qu'est une logique sous ses aspects sémantiques. On peut citer les exemples suivants.\\\\
...
\end{itemize}
Parmi les autres institutions connues, il y a la logique modale propositionnelle $\gr{MPL}$ et ses généralisations métrique $\gr{MMPL}$ et topologique $\gr{TMPL}$. Tout comme pour $\gr{FOL}^\gr{1}$, la définition de la relation de satisfaction n'est pas naturelle : l'absence de notion d'état force à quantifier universellement. On aimerait pouvoir dire que $\M \models_{\La} \ph[a_1...a_n]$ seulement pour certains $n$-uples $a_1...a_n$ (dans $\gr{FOL}^\gr{1}$) ou que $(I,W,R) \models_P^i \ph$ seulement pour certains mondes possibles indicés par $J \subsetneq I$ (dans $\gr{MPL}$). Ces considérations ont fait émerger la notion d'institution stratifiée.
\newpage
